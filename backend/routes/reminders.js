...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const row = await db.get('SELECT * FROM recordatorios WHERE id = ?', [id]);
...
router.put('/:id', async (req, res) => {
  ...
  try {
    ...
    const row = await db.get('SELECT * FROM recordatorios WHERE id = ?', [req.params.id]);
...
router.delete('/:id', async (req, res) => {
  try {
    const result = await db.run('DELETE FROM recordatorios WHERE id = ?', [req.params.id]);
    if (result.changes === 0) {
      return res.status(404).json({ error: 'Recordatorio no encontrado' });
    }
